...
性能监控是前端工程化的"血压计"——通过采集 FCP、LCP、CLS、INP 等 Core Web Vitals，结合 Performance API 和自定义测速点，把"页面快不快"从感受变成可量化、可报警、可优化的数据。

## 核心知识点

### 1. Core Web Vitals 四大指标

| 指标 | 含义 | 优质标准 | 测量的是 |
|------|------|---------|---------|
| **LCP** | 最大内容绘制 | ≤ 2.5s | "页面看起来加载完了吗" |
| **INP** | 交互到下次绘制 | ≤ 200ms | "点击后需要多久才响应" |
| **CLS** | 累计布局偏移 | ≤ 0.1 | "页面有没有突然跳来跳去" |
| **FCP** | 首次内容绘制 | ≤ 1.8s | "用户最早看到内容的时间" |
