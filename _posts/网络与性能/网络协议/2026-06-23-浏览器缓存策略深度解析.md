...
**❌ 误区三："ETag 默认用文件的 inode + mtime + size 生成"**

nginx 默认确实是 `Last-Modified + Content-Length` 组合成 ETag（不是 inode）。但实际部署跨多台服务器时，不同机器的 inode 和 mtime 可能不同，导致同一文件不同服务器 ETag 不一样。**跨服务器部署要关掉 ETag 或改为内容哈希生成**。
