...
### 6.1 Pick vs Omit 的约束差异

```typescript
// Pick 的约束：K 必须是 T 的键
type Pick<T, K extends keyof T>  // K ⊂ keyof T → 安全

// Omit 的约束：K 只需是合法键名
type Omit<T, K extends keyof any> // 更宽松 — 即使 K 包含 T 中没有的属性也不会报错
```

为什么 Omit 更宽松？因为内部用 `Exclude<keyof T, K>` 过滤——不存在的属性直接被忽略。如果约束为 `K extends keyof T`，想排除一个不存在于 T 中的属性就会报错，降低了灵活性。

### 6.2 手写对照速查表

| 工具类型 | 手写关键 | 核心机制 | 面试必说 |
|---------|---------|---------|--------|
| `Partial<T>` | `[P in keyof T]?: T[P]` | 映射 + 可选修饰符 | `?` 添加可选 |
| `Required<T>` | `[P in keyof T]-?: T[P]` | 映射 + 移除可选 | `-?` 移除可选 |
| `Readonly<T>` | `[P in keyof T]: T[P]` + readonly | 映射 + 只读 | `readonly` 关键词 |
| `Pick<T,K>` | `[P in K]: T[P]` + K extends keyof T | 映射 + 索引 | K 必须是 T 的键 |
| `Omit<T,K>` | `Pick<T, Exclude<keyof T, K>>` | Exclude + Pick 组合 | 组合思想 |
| `Record<K,T>` | `[P in K]: T` + K extends keyof any | 异态映射 | 键集合不依赖源类型 |
