...
变长元组（Variadic Tuples）是 TypeScript 4.0 的标志性特性。它的本质是**在元组类型中引入展开运算符（spread）的泛型支持**。
...
// 当 T = [Promise<number>, Promise<string>] 时：
// keyof T = '0' | '1' | 'length' | 'pop' | ... // 数组方法名！
// 所以映射类型会遍历所有数组方法名！
// 但 TypeScript 做了特殊处理——homomorphic mapped types on tuple types
// 编译器检测到这是一个同态映射（{ [K in keyof T]: T[K] } 形式）
// 会保留元组的 "rest" 结构

// 所以：
// MapTuple<[Promise<number>, Promise<string>]>
// = [number, string]  ✅（保留了元组结构）
```

### 6.2 TypeScript 类型递归的性能局限

类型递归在编译时是"真递归"——编译器需要展开完整的递归链，直到终止条件。

```typescript
type Path<T, Prefix extends string = ''> = 
  T extends Record<string, unknown>
    ? { [K in keyof T & string]: `${Prefix}${K}` | Path<T[K], `${Prefix}${K}.`> }[keyof T & string]
    : never;

// 对于有 10 个属性、5 层深度的对象：
// 递归展开的节点数 = 10 + 10*10 + 10*10*10 + ... = 111110 个节点
// 虽然 TypeScript 有缓存，但仍然需要大量计算

// 优化：在深度过大时截断
type PathLimited<T, Depth extends number = 10, _Current extends 0[] = []> = 
  _Current['length'] extends Depth
    ? `${string}` // 超过深度限制，退化到 string
    : T extends Record<string, unknown>
      ? {
          [K in keyof T & string]:
            | `${Prefix & string}${K}`
            | PathLimited<T[K], Depth, [..._Current, 0]>;
        }[keyof T & string]
      : never;
```

### 6.3 柯里化类型中的函数参数长度（arity）

柯里化依赖函数的 `length` 属性（参数数量）：

```javascript
function fn1(a: number, b: string) {}
fn1.length; // 2

function fn2(a: number, b: string = 'default') {}
fn2.length; // 1（默认参数不计入）

function fn3(...args: string[]) {}
fn3.length; // 0（rest 参数不计入）
```

在类型层面，我们可以通过 `Parameters<T>['length']` 获取参数数量，但这无法区分 `[string, number]` 和 `[string, number, ...string[]]`（后者有 rest 参数）。

**完整的柯里化类型需要处理 rest 参数**：

```typescript
type CurryWithRest<F extends (...args: any) => any> = 
  F extends (...args: infer P) => infer R
    ? P extends [infer A, ...infer Rest]
      ? Rest extends []
        ? (arg: A) => R
        : Rest extends [...infer Last]
          ? // 判断是否是 rest 参数
            Rest extends [infer L, ...any[]]
            ? L extends any[] 
              ? (arg: A) => CurryWithRest<(...args: Rest[0]) => R>
              : (arg: A) => CurryWithRest<(...args: Rest) => R>
            : never
      : () => R
    : never;
```
