...
| 操作 | 工具类型 | 原理 |
|------|---------|------|
| **过滤** `'a' \| 'b' \| 'c'` → `'a'` | `Extract<T, U>` | 分布式条件类型保留匹配的 |
| **排除** `'a' \| 'b' \| 'c'` → `'b' \| 'c'` | `Exclude<T, U>` | 分布式条件类型剔除匹配的 |
| **全选** `{a:1} \| {b:2}` → `{a:1} & {b:2}` | `UnionToIntersection<U>` | 利用函数参数逆变 infer |
| **映射**`({a:1} \| {b:2})` → `Readonly`版 | 分布式 + 映射组合 | `U extends any ? Readonly<U> : never` |

### 6.2 分布式条件类型硬限制

- **裸参数**才分布：`T extends U ? X : Y` 中 T 是裸参数 → 分布
- **`[T]` 包装**阻止分布：`[T] extends [U] ? X : Y` → 整体判断
- **`never` 空联合**返回 `never`：`Test<never>` → `never`（不是 `X|Y`）

### 6.3 联合类型窄化三板斧

```typescript
// 1. in 操作符 — 按属性名窄化
if ('fly' in pet) { pet /* Bird */ }

// 2. typeof / instanceof — 按运行时类型窄化
if (typeof val === 'string') { val /* string */ }

// 3. 可辨识联合 — 按字面量字段窄化（最推荐）
type Result = { status: 'success'; data: unknown } | { status: 'error'; error: Error };
if (result.status === 'success') { result /* success 分支 */ }
```

### 6.4 联合类型的类型兼容性规则（面试常考）

```typescript
type R1 = 'a' extends string ? true : false; // true
type R2 = ('a' | 1) extends (string | number) ? true : false; // true（联合到联合兼容）
type R3 = { a: number; b: string } extends { a: number } | { b: string } ? true : false; // true（交叉是联合的子类型）
type R4 = never extends string ? true : false; // true（never 是底类型）
```
