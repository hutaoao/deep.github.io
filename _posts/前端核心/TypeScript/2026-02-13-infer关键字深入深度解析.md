...
把 `infer` 想象成"类型世界的正则捕获组"——条件类型定义了匹配规则，`infer` 负责把匹配到的片段捞出来、给它起个名字、交给后续代码使用。它**不创造类型，只是发现并命名已有结构中隐藏的部分**。

**几点关键理解**：
- `infer` 声明的类型变量**仅在当前条件分支（`?` 和 `:` 之间）有效**
- 嵌套条件类型中，内层 `infer` 覆盖外层同名变量（就近原则）
- 同一层级多个 `infer` 是**并行匹配**，不是串行依赖
...
- **`infer` + 映射类型**：实现 `DeepPick<T, K>` 等深层属性选择工具（面试加分项，非必须）
- **`infer` 在声明文件中的运用**：理解内置工具类型的实现思路
