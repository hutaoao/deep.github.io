...
深层递归类型会在编译时产生显著的性能开销。来看一个基准：

```typescript
// 测试嵌套深度
interface Level0 { x: number; }
interface Level1 { a: Level0; }
interface Level2 { b: Level1; }
// ... 一直到 Level50

type Deep50 = DeepReadonly<Level50>;
```

对于 50 层嵌套的 `DeepReadonly`，TypeScript 编译器需要：
- 生成 50 层映射类型
- 每层对 keyof 进行求值（O(1)）
- 创建约 50 个匿名类型

在 TypeScript 5.x 上，这个操作可以在 5-20ms 内完成。但如果同时有多个 DeepReadonly 实例，或与其他复杂类型组合，编译时间会显著增加。

**优化建议**：
1. 使用 `type DeepReadonly<T> = T extends object ? { readonly [P in keyof T]: DeepReadonly<T[P]> } : T;` 用条件类型先判断再递归
2. 限制递归深度（如 TypeScript 5.x 的 `--typeRecursionLimit`）
3. 避免在热路径中使用过深的递归类型

### 6.4 TypeScript 5.x 的递归优化

TypeScript 5.0+ 对递归类型做了重要改进：

**优化 1：缓存实例化结果**

```typescript
// TypeScript 5.0+ 会缓存 DeepReadonly<User> 的结果
// 如果多次使用 DeepReadonly<User>，只计算一次
```
...
```typescript
// 旧版：深度优先
// DeepReadonly<{a: {b: {c: number}}}> → 先完整展开 a 再展开 b

// 新版：广度优先（TS 5.0+）
// 先展开第二层所有属性，再展开第三层
```

**优化 3：类型展平（Type Flattening）**

TypeScript 5.x 对简单的映射类型做了扁平化处理，减少了中间类型数量。
