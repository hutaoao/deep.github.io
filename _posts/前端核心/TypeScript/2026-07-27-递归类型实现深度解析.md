...
| 要素 | 说明 | 示例 |
|------|------|------|
| **终止条件** | 非对象/基础类型 → 直接返回 | `T extends Record<string,unknown> ? ... : T` |
| **递归步骤** | 属性逐个递归处理 | `[P in keyof T]: DeepX<T[P]>` |
| **边界处理** | 数组/函数/Date 等特殊类型 | `T extends Array<infer U> ? ReadonlyArray<DeepX<U>>` |
...
```typescript
type DeepX<T> =
  T extends Array<infer U> ? ReadonlyArray<DeepX<U>>  // 1. 数组优先
  : T extends Function ? T                              // 2. 函数保持原样
  : T extends Date | RegExp ? T                         // 3. 内置对象
  : T extends Record<string, unknown> ? DeepX<T>        // 4. 纯对象递归
  : T;                                                   // 5. 基础类型终止
```

> 为什么数组必须在纯对象前面？因为 JavaScript 中 `Array instanceof Object === true`，数组先经过对象分支会导致 `T[P]` 作用于索引下标。

### 6.3 性能边界（面试够用版）

- **递归深度上限 ≈ 50 层**（超过报 `TS2589`）
- 日常业务 3-5 层完全无感
- 高频递归类型有编译器缓存，重复使用不额外消耗
- **面试被问到**：说「知道有 50 层限制」就够了，不需要提编译器实现细节
