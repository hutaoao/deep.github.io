...
## 六、进阶面试必考：infer 的高级陷阱

### 6.1 逆变位置 infer → 交叉类型

函数参数是**逆变**的——当 infer 在参数位置时，多个函数类型的参数会被推断为交叉类型而非联合类型。这就是 `UnionToIntersection` 的实现原理：
...
type T1 = UnionToIntersection<{ a: number } | { b: string }>;
// { a: number } & { b: string }
```

### 6.2 never 在条件类型中的特殊行为

`never` 作为空联合，在条件类型中有两个反直觉行为：

1. **分布式条件类型对空联合直接短路**：`TestNever<never>` 返回 `never` 而非 `'yes' | 'no'`
2. **`[T]` 包裹后恢复正常判断**：`[T] extends [never] ? true : false` 返回 `true`（面试用来检测 `never` 类型！）
