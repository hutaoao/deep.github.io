---
layout: post
title: "CommonJS 规范与实现深度解析"
description: >
  CommonJS 三要点：同步加载、模块缓存（同一文件只执行一次）、值拷贝导出（导出值不随模块内变化更新）。
  理解这三点就能解释 Node 模块的几乎所有行为。
date: 2026-01-31
...
**第 3 步是理解循环依赖的钥匙：** 缓存设在执行代码**之前**——先占位 `{}`，代码跑完再填充。循环依赖方拿到的是半成品空对象而不是 `undefined`。
